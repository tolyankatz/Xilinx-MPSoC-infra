...
**Purpose**: Manual execution commands for complete system validation
...
# Copy to container
docker cp /tmp/create-repo.json embedded-ci-lab-simulator-host_controller-1:/tmp/create-repo.json
...
#### Push to Repository
```bash
cd /tmp/zcu102-bsp-validation-monorepo
...
docker-compose exec host_controller curl -s http://gitea:3000/api/v1/version | head -n 1
docker-compose exec host_controller curl -s http://jenkins:8080/login | head -n 1
...
# Verify artifacts
docker-compose exec host_controller mc ls minio/test-logs | tail -n 5
```
...
docker-compose logs host_controller | grep -i error
docker-compose logs dut_simulator | grep -i error
```
...
docker-compose exec host_controller mc ls minio/test-logs | tail -n 1
```
...
**Validated**: All commands tested and functional
